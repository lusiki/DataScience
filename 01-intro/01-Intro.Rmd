...
### Course
<br>
`r fa('globe')` https://github.com/lusiki/DataScience
...
`r fa('address-book')` [Luka Sikic](https://grantmcdermott.com/)

`r fa('envelope')` [luka.sikic@unicath.hr](mailto:luka.sikic@unicath.hr)

`r fa('graduation-cap')` Assistant Professor (statistics)
...
# Why this course?
<br>
Fill in the gaps left by traditional statistics and methods classes.
...
Data science skills are largely distinct from (and complementary to) the core 'metrics oeuvre familiar to social scientists.
...
- Oral exam.
- Presentation to summarize either a key lecture reading, or an (approved) software package/platform.
...
PS &mdash; I'll also award a class participation bonus at my discretion.
...
```{bash eval=FALSE}
  ...
  git config --global user.email 'luka.sikic@unicath.hr'
...
```{r gen_pdf, include = FALSE, cache = FALSE, eval = F}
